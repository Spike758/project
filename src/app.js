...
const inputAddTariff = document.getElementById("addTariff");

inputAddTariff.addEventListener("click", () => {
  modalUpdate();
  modal.innerHTML = `
      <div class="top">
    ...
    table_tariff.forEach((item) => {
      let new_row = `
        <div class="row">
          ...
          <div class="col"><h3>${item.licenses}</h3></div>
          <div class="col"><h3>${item.tracks}</h3></div>
          <div class="col"><h3>${item.price}</h3></div>
          <div class="col bd-r action">
            <div class="square pen">
...
const deleteRow = (e) => {
  const row = e.parentNode.parentNode.parentNode.querySelector(".row");
  row.remove();
  console.log(row);
};
